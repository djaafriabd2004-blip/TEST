...
async def send_message_with_retry(send_func, *args, retries=3, delay=1.5, **kwargs):
    ...
    Falls back to plain text if Markdown/HTML or MessageEntity formatting fails.
    """
    for attempt in range(1, retries + 1):
        try:
            ...
            err_str = str(e).lower()
            if ("can't parse entities" in err_str or "bad request" in err_str or "entity" in err_str or "entities" in err_str) and ("parse_mode" in kwargs or "entities" in kwargs):
                kwargs_no_pm = kwargs.copy()
                kwargs_no_pm.pop("parse_mode", None)
                kwargs_no_pm.pop("entities", None)
                try:
                    return await send_func(*args, **kwargs_no_pm)
                except Exception as inner_e:
                    logger.warning(f"Fallback without parse_mode/entities failed: {inner_e}")
